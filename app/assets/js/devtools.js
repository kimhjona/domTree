...
chrome.devtools.panels.create('DejaVue', 'assets/img/logo.png', 'index.html', function (extensionPanel) {
    ...
    chrome.storage.sync.get('oldUrl', function (data) {
        oldUrl = data.oldUrl
    });
    ...
    show.then(function (_panelWindow) {
        ...
        function updater() {
            let poller = setInterval(function () {
                chrome.storage.sync.get('urlz', function (data) {
                    urlz = data.urlz
                })
                if (urlz !== oldUrl) {
                    oldUrl = urlz;
                    chrome.devtools.inspectedWindow.eval(
                        `    
                          console.log('hi');
                          let data = [];
                          let addDVChecker = document.querySelectorAll("*");
                          console.log('addDVChecker', addDVChecker);
                          addDVChecker.forEach((elem, ind) => elem.classList.add('domtree' + ind)); 

                  

                          function nodeConstructor (node) {
                            this.name: node.id,
                            this.parent: node.parentElement.classList[classList.length - 1]
                          }

                          addDVChecker.forEach((elem) => {
                            data.push(new nodeConstructor(elem));
                          })
